...
- `apps/server` 是本地 gateway，提供 tRPC API、SSE、SQLite store 和 active awesome 上下文。
- `apps/web` 是本地 operator UI，通过 tRPC 和 SSE 操作 gateway。
...
4. Web 创建 geek task 时，server 先持久化 task 和 ultrawork 关联，再调用 `GeekRuntime.launch`。
5. Runtime 产生 task event，server 通过 `/events` SSE 推给 web，同时 web 可用 recovery query 重建状态。
