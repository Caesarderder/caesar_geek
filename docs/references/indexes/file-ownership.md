...
| `CODE:apps/server/src/app.ts` | backend | Fastify/tRPC app、API router、SSE、active awesome | manual-read |
| `CODE:apps/server/src/db/sqlite.ts` | backend | RegistryStore、AwesomeStore、SQLite schema 和 mapping | manual-read |
| `CODE:apps/server/src/index.ts` | backend | server 启动入口 | manual-read |
| `CODE:apps/web/src/main.tsx` | frontend | operator UI、tRPC client、SSE listener | manual-read |
| `CODE:apps/web/src/styles.css` | frontend | UI 样式 | path-exists |
| `CODE:apps/web/vite.config.ts` | frontend | Vite React 和 API proxy | manual-read |
| `CODE:packages/shared/src/index.ts` | shared | Zod schema、types、policy classifier | manual-read |
